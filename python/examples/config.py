...
Demonstrates: kvstore_open_v2 via Store keyword args,
...
def delete_mode():
    ...
    with Store(DB_FILE2, journal_mode=JOURNAL_DELETE) as db:
...
def busy_timeout():
    ...
    with Store(DB_FILE, busy_timeout=5000) as db:
        db["shared"] = "data"
...
def read_only_mode():
    ...
    with Store(DB_FILE, read_only=1) as db:
...
def wal_size_limit():
    ...
    with Store(DB_FILE, wal_size_limit=50) as db:
        for i in range(200):
