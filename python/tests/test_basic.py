...
def db_delete(tmp_path):
    """Store opened in DELETE (rollback) journal mode."""
    path = str(tmp_path / "delete.db")
    with Store(path, journal_mode=JOURNAL_DELETE) as store:
        yield store
...
def test_checkpoint_wal(tmp_path):
    ...
    with Store(path, journal_mode=JOURNAL_WAL) as db:
        for i in range(50):
...
def test_open_v2_via_kwargs(tmp_path):
    ...
    with Store(path, cache_size=500, busy_timeout=100) as db:
        db.put(b"k", b"v")
...
def test_open_delete_mode(tmp_path):
    ...
    with Store(path, journal_mode=JOURNAL_DELETE) as db:
