...
static void test_edge_empty_value(void) {
    ...
    int rc = keyvaluestore_open(DB_FILE, &kv, KEYVALUESTORE_JOURNAL_DELETE);
    int ok = 0;

    if (rc == KEYVALUESTORE_OK) {
        const char* k = "empty_val_key";
        rc = keyvaluestore_put(kv, k, (int)strlen(k), "", 0);
        if (rc == KEYVALUESTORE_OK) {
            void* got = NULL; int glen = 0;
            rc = keyvaluestore_get(kv, k, (int)strlen(k), &got, &glen);
            ok = (rc == KEYVALUESTORE_OK && glen == 0);
...
static void test_edge_binary_key_with_nulls(void) {
    ...
    if (rc == KEYVALUESTORE_OK) {
        ...
        rc = keyvaluestore_put(kv, binkey, 5, val, (int)strlen(val));
        if (rc == KEYVALUESTORE_OK) {
            void* got = NULL; int glen = 0;
            rc = keyvaluestore_get(kv, binkey, 5, &got, &glen);
            ok = (rc == KEYVALUESTORE_OK && glen == (int)strlen(val)
...
static void test_edge_single_byte_key(void) {
    ...
    if (rc == KEYVALUESTORE_OK) {
        ...
        rc = keyvaluestore_put(kv, &k, 1, val, (int)strlen(val));
        if (rc == KEYVALUESTORE_OK) {
            void* got = NULL; int glen = 0;
            rc = keyvaluestore_get(kv, &k, 1, &got, &glen);
            ok = (rc == KEYVALUESTORE_OK && glen == (int)strlen(val)
                && memcmp(got, val, glen) == 0);
            if (got) sqliteFree(got);
        }
...
static void test_edge_large_key_value(void) {
    ...
    if (rc == KEYVALUESTORE_OK) {
        ...
        if (bigkey && bigval) {
            ...
            rc = keyvaluestore_put(kv, bigkey, klen, bigval, vlen);
            if (rc == KEYVALUESTORE_OK) {
                void* got = NULL; int glen = 0;
                rc = keyvaluestore_get(kv, bigkey, klen, &got, &glen);
                ok = (rc == KEYVALUESTORE_OK && glen == vlen
...
static void test_edge_overwrite_same_key(void) {
    ...
    if (rc == KEYVALUESTORE_OK) {
        ...
        for (i = 0; i < 100; i++) {
            ...
            rc = keyvaluestore_put(kv, k, (int)strlen(k), val, (int)strlen(val));
            if (rc != KEYVALUESTORE_OK) break;
        }
        if (rc == KEYVALUESTORE_OK) {
            void* got = NULL; int glen = 0;
            rc = keyvaluestore_get(kv, k, (int)strlen(k), &got, &glen);
            ok = (rc == KEYVALUESTORE_OK && glen == (int)strlen("version-99")
...
static void test_edge_get_nonexistent(void) {
    ...
    if (rc == KEYVALUESTORE_OK) {
        void* got = NULL; int glen = 0;
        rc = keyvaluestore_get(kv, "no_such_key", 11, &got, &glen);
        ok = (rc == KEYVALUESTORE_NOTFOUND && got == NULL);
        keyvaluestore_close(kv);
    }
...
static void test_edge_delete_nonexistent(void) {
    ...
    if (rc == KEYVALUESTORE_OK) {
        rc = keyvaluestore_delete(kv, "no_such_key", 11);
        ok = (rc == KEYVALUESTORE_NOTFOUND);
        keyvaluestore_close(kv);
    }
...
static void test_edge_put_after_delete(void) {
    ...
    if (rc == KEYVALUESTORE_OK) {
        ...
        rc = keyvaluestore_put(kv, k, (int)strlen(k), "first", 5);
        if (rc == KEYVALUESTORE_OK) rc = keyvaluestore_delete(kv, k, (int)strlen(k));
        if (rc == KEYVALUESTORE_OK) rc = keyvaluestore_put(kv, k, (int)strlen(k), "second", 6);
        if (rc == KEYVALUESTORE_OK) {
            void* got = NULL; int glen = 0;
            rc = keyvaluestore_get(kv, k, (int)strlen(k), &got, &glen);
            ok = (rc == KEYVALUESTORE_OK && glen == 6 && memcmp(got, "second", 6) == 0);
            if (got) sqliteFree(got);
        }
        keyvaluestore_close(kv);
    }
...
static void test_write_storm(int journalMode, const char* label) {
    const char* db = (journalMode == KEYVALUESTORE_JOURNAL_WAL) ? DB_WAL_FILE : DB_FILE;
    cleanup(db);
    KeyValueStore* kv = NULL;
    int rc = keyvaluestore_open(db, &kv, journalMode);
    int ok = 0;
    ...
    if (rc == KEYVALUESTORE_OK) {
        ...
        for (i = 0; i < WRITE_STORM_N; i++) {
            ...
            if ((i % 25000) == 0 && i > 0) {
                ...
                char* errMsg = NULL;
                int ic = keyvaluestore_integrity_check(kv, &errMsg);
                if (ic != KEYVALUESTORE_OK) {
        ...
        if (rc == KEYVALUESTORE_OK) keyvaluestore_commit(kv);
        ...
        for (i = 0; i < 4; i++) {
            ...
            void* got = NULL; int glen = 0;
            rc = keyvaluestore_get(kv, key, (int)strlen(key), &got, &glen);
            if (rc != KEYVALUESTORE_OK || glen != (int)strlen(val)
                || memcmp(got, val, glen) != 0) {
                spotOk = 0;
            }
            if (got) sqliteFree(got);
        ...
        ok = (rc == KEYVALUESTORE_OK && integrityOk && spotOk);
        keyvaluestore_close(kv);
    }
...
static void test_large_dataset(void) {
    ...
    if (rc == KEYVALUESTORE_OK) {
        ...
        rc = keyvaluestore_iterator_create(kv, &iter);
        int count = 0;
        if (rc == KEYVALUESTORE_OK) {
            rc = keyvaluestore_iterator_first(iter);
...
static void test_crash_recovery_uncommitted(void) {
    ...
    rc = keyvaluestore_open(DB_FILE, &kv, KEYVALUESTORE_JOURNAL_DELETE);
    if (rc != KEYVALUESTORE_OK) { print_result("Crash recovery: uncommitted txn", 0); return; }
    keyvaluestore_put(kv, "safe", 4, "committed_data", 14);
    keyvaluestore_close(kv);

    rc = keyvaluestore_open(DB_FILE, &kv, KEYVALUESTORE_JOURNAL_DELETE);
    if (rc != KEYVALUESTORE_OK) { print_result("Crash recovery: uncommitted txn", 0); return; }
    keyvaluestore_begin(kv, 1);
    keyvaluestore_put(kv, "unsafe", 6, "uncommitted_data", 16);
    keyvaluestore_close(kv);
    ...
    void* got = NULL; int glen = 0;
    rc = keyvaluestore_get(kv, "safe", 4, &got, &glen);
    int safeOk = (rc == KEYVALUESTORE_OK && glen == 14 && memcmp(got, "committed_data", 14) == 0);
    if (got) sqliteFree(got);

    int unsafeExists = 0;
    keyvaluestore_exists(kv, "unsafe", 6, &unsafeExists);
...
static void test_crash_recovery_wal(void) {
    ...
    rc = keyvaluestore_open(DB_WAL_FILE, &kv, KEYVALUESTORE_JOURNAL_WAL);
    if (rc != KEYVALUESTORE_OK) { print_result("Crash recovery WAL: uncommitted txn", 0); return; }
    keyvaluestore_put(kv, "wal_safe", 8, "wal_committed", 13);
    keyvaluestore_close(kv);

    rc = keyvaluestore_open(DB_WAL_FILE, &kv, KEYVALUESTORE_JOURNAL_WAL);
    if (rc != KEYVALUESTORE_OK) { print_result("Crash recovery WAL: uncommitted txn", 0); return; }
    keyvaluestore_begin(kv, 1);
    keyvaluestore_put(kv, "wal_unsafe", 10, "wal_uncommitted", 15);
    keyvaluestore_close(kv);
    ...
    void* got = NULL; int glen = 0;
    rc = keyvaluestore_get(kv, "wal_safe", 8, &got, &glen);
    int safeOk = (rc == KEYVALUESTORE_OK && glen == 13);
    if (got) sqliteFree(got);

    int unsafeExists = 0;
    keyvaluestore_exists(kv, "wal_unsafe", 10, &unsafeExists);
...
static void test_rapid_open_close(void) {
    ...
    rc = keyvaluestore_open(DB_FILE, &kv, KEYVALUESTORE_JOURNAL_DELETE);
    if (rc != KEYVALUESTORE_OK) { print_result("Rapid open/close (200 cycles)", 0); return; }
    keyvaluestore_put(kv, "persist", 7, "survives", 8);
    keyvaluestore_close(kv);

    int i;
    for (i = 0; i < cycles; i++) {
        rc = keyvaluestore_open(DB_FILE, &kv, KEYVALUESTORE_JOURNAL_DELETE);
        if (rc != KEYVALUESTORE_OK) { ok = 0; break; }

        void* got = NULL; int glen = 0;
        rc = keyvaluestore_get(kv, "persist", 7, &got, &glen);
        if (rc != KEYVALUESTORE_OK || glen != 8 || memcmp(got, "survives", 8) != 0) {
            ok = 0;
            if (got) sqliteFree(got);
            keyvaluestore_close(kv);
            break;
        ...
        if ((i % 50) == 0) {
            ...
            keyvaluestore_put(kv, "cycle_key", 9, val, (int)strlen(val));
        }
...
static void test_cross_config(void) {
    int modes[] = { KEYVALUESTORE_JOURNAL_DELETE, KEYVALUESTORE_JOURNAL_WAL };
    ...
    for (m = 0; m < 2; m++) {
        cleanup(dbs[m]);
        KeyValueStore* kv = NULL;
        int rc = keyvaluestore_open(dbs[m], &kv, modes[m]);
        if (rc != KEYVALUESTORE_OK) { allOk = 0; continue; }

        const char* k = "xconfig";
        rc = keyvaluestore_put(kv, k, 7, "v1", 2);
        if (rc != KEYVALUESTORE_OK) { allOk = 0; keyvaluestore_close(kv); continue; }

        void* got = NULL; int glen = 0;
        rc = keyvaluestore_get(kv, k, 7, &got, &glen);
        if (rc != KEYVALUESTORE_OK || glen != 2 || memcmp(got, "v1", 2) != 0) allOk = 0;
        if (got) { sqliteFree(got); } got = NULL;

        rc = keyvaluestore_put(kv, k, 7, "v2-updated", 10);
        if (rc != KEYVALUESTORE_OK) allOk = 0;

        rc = keyvaluestore_get(kv, k, 7, &got, &glen);
        if (rc != KEYVALUESTORE_OK || glen != 10 || memcmp(got, "v2-updated", 10) != 0) allOk = 0;
        if (got) { sqliteFree(got); } got = NULL;

        rc = keyvaluestore_delete(kv, k, 7);
        if (rc != KEYVALUESTORE_OK) allOk = 0;

        int exists = 0;
        keyvaluestore_exists(kv, k, 7, &exists);
        if (exists) allOk = 0;

        rc = keyvaluestore_put(kv, k, 7, "v3-reinsert", 11);
        if (rc != KEYVALUESTORE_OK) allOk = 0;

        keyvaluestore_begin(kv, 1);
        keyvaluestore_put(kv, "txn1", 4, "committed", 9);
        keyvaluestore_commit(kv);

        rc = keyvaluestore_get(kv, "txn1", 4, &got, &glen);
        if (rc != KEYVALUESTORE_OK || glen != 9) allOk = 0;
        if (got) { sqliteFree(got); } got = NULL;

        keyvaluestore_begin(kv, 1);
        keyvaluestore_put(kv, "txn2", 4, "rolled_back", 11);
        keyvaluestore_rollback(kv);

        rc = keyvaluestore_get(kv, "txn2", 4, &got, &glen);
        if (rc != KEYVALUESTORE_NOTFOUND) allOk = 0;
        if (got) { sqliteFree(got); } got = NULL;
...
static int open_with_retry(const char* db, KeyValueStore** ppKV, int journalMode) {
    int rc, retries = 0;
    do {
        rc = keyvaluestore_open(db, ppKV, journalMode);
...
static void* writer_thread(void* arg) {
    ...
    if (rc != KEYVALUESTORE_OK) { ta->errors++; ta->lastErrCode = rc; return NULL; }
    ...
    for (i = 0; i < ta->ops; i++) {
        ...
        do {
            rc = keyvaluestore_put(kv, key, (int)strlen(key), val, (int)strlen(val));
            if (rc == KEYVALUESTORE_OK) { ok = 1; break; }
...
static void* reader_thread(void* arg) {
    ...
    for (i = 0; i < ta->ops; i++) {
        ...
        if (rc == KEYVALUESTORE_OK) {
            ...
            int n = 0;
            while (!keyvaluestore_iterator_eof(iter) && n < 10) {
                keyvaluestore_iterator_next(iter);
                n++;
            }
            keyvaluestore_iterator_close(iter);
...
static void test_concurrent_stress(void) {
    ...
    KeyValueStore* kv = NULL;
    int rc = keyvaluestore_open(DB_WAL_FILE, &kv, KEYVALUESTORE_JOURNAL_WAL);
    if (rc != KEYVALUESTORE_OK) { print_result("Concurrent stress (WAL, 8 threads)", 0); return; }
    keyvaluestore_put(kv, "seed", 4, "initial", 7);
    keyvaluestore_close(kv);
    ...
    for (i = 0; i < NUM_THREADS; i++) {
        ...
        args[i].journalMode = KEYVALUESTORE_JOURNAL_WAL;
        if (i < 2) {
    ...
    usleep(50000);
    rc = open_with_retry(DB_WAL_FILE, &kv, KEYVALUESTORE_JOURNAL_WAL);
    int intOk = 0;
    if (rc == KEYVALUESTORE_OK) {
        char* errMsg = NULL;
        int ic = keyvaluestore_integrity_check(kv, &errMsg);
        intOk = (ic == KEYVALUESTORE_OK);
    ...
    if (rc == KEYVALUESTORE_OK) {
        ...
        if (rc == KEYVALUESTORE_OK) {
            ...
            while (!keyvaluestore_iterator_eof(iter)) {
                ...
                keyvaluestore_iterator_key(iter, &ik, &ikn);
                keyvaluestore_iterator_value(iter, &iv, &ivn);
                if (!ik || ikn <= 0) keyErr++;
                if (!iv || ivn <= 0) keyErr++;
                count++;
                keyvaluestore_iterator_next(iter);
...
static void test_iterator_empty_db(void) {
    ...
    if (rc == KEYVALUESTORE_OK) {
        ...
        if (rc == KEYVALUESTORE_OK) {
            keyvaluestore_iterator_first(iter);
            ok = (keyvaluestore_iterator_eof(iter) == 1);
            keyvaluestore_iterator_close(iter);
        }
        keyvaluestore_close(kv);
    }
...
static void test_transaction_cycling(void) {
    ...
    if (rc == KEYVALUESTORE_OK) {
        ...
        for (i = 0; i < TXN_CYCLE_N; i++) {
            ...
            if (rc != KEYVALUESTORE_OK) { errors++; continue; }

            make_key(key, sizeof(key), i);
            snprintf(val, sizeof(val), "txn-%d", i);
            rc = keyvaluestore_put(kv, key, (int)strlen(key), val, (int)strlen(val));
            if (rc != KEYVALUESTORE_OK) { errors++; keyvaluestore_rollback(kv); continue; }

            rc = keyvaluestore_commit(kv);
            if (rc != KEYVALUESTORE_OK) errors++;
        }
        ...
        void* got = NULL; int glen = 0;
        rc = keyvaluestore_get(kv, key, (int)strlen(key), &got, &glen);
        int lastOk = (rc == KEYVALUESTORE_OK && glen > 0);
        if (got) sqliteFree(got);
...
static void test_rollback_cycling(void) {
    ...
    if (rc == KEYVALUESTORE_OK) {
        keyvaluestore_put(kv, "anchor", 6, "stable", 6);
        ...
        for (i = 0; i < 1000; i++) {
            rc = keyvaluestore_begin(kv, 1);
            if (rc != KEYVALUESTORE_OK) { errors++; continue; }
            keyvaluestore_put(kv, "ephemeral", 9, "gone", 4);
            rc = keyvaluestore_rollback(kv);
            if (rc != KEYVALUESTORE_OK) errors++;
        }

        void* got = NULL; int glen = 0;
        rc = keyvaluestore_get(kv, "anchor", 6, &got, &glen);
        int anchorOk = (rc == KEYVALUESTORE_OK && glen == 6);
        if (got) sqliteFree(got);

        int ephExists = 0;
        keyvaluestore_exists(kv, "ephemeral", 9, &ephExists);
    ...
    if (rc == KEYVALUESTORE_OK) {
        ...
        for (i = 0; i < MIXED_OPS_N; i++) {
            ...
            if (op < 40) {
                ...
                void* got = NULL; int glen = 0;
                rc = keyvaluestore_get(kv, key, (int)strlen(key), &got, &glen);
                if (rc != KEYVALUESTORE_OK && rc != KEYVALUESTORE_NOTFOUND) errors++;
                if (got) sqliteFree(got);
                gets++;
            }
            else if (op < 85) {
                ...
                int e = 0;
                rc = keyvaluestore_exists(kv, key, (int)strlen(key), &e);
                if (rc != KEYVALUESTORE_OK) errors++;
                exists_ops++;
            }
...
static void test_cf_stress(void) {
    ...
    if (rc == KEYVALUESTORE_OK) {
        int nCF = 10;
        KeyValueColumnFamily* cfs[10];
        int i, j;
        char cfname[32], key[32], val[64];

        for (i = 0; i < nCF; i++) {
            snprintf(cfname, sizeof(cfname), "stress_cf_%d", i);
            rc = keyvaluestore_cf_create(kv, cfname, &cfs[i]);
            if (rc != KEYVALUESTORE_OK) { print_result("Column family stress (10 CFs)", 0); keyvaluestore_close(kv); return; }
        }
        ...
        for (i = 0; i < nCF; i++) {
            for (j = 0; j < 500; j++) {
                ...
                rc = keyvaluestore_cf_put(cfs[i], key, (int)strlen(key), val, (int)strlen(val));
                if (rc != KEYVALUESTORE_OK) break;
            }
            if (rc != KEYVALUESTORE_OK) break;
        }
        ...
        int allCountOk = 1;
        for (i = 0; i < nCF && rc == KEYVALUESTORE_OK; i++) {
            KeyValueIterator* iter = NULL;
            rc = keyvaluestore_cf_iterator_create(cfs[i], &iter);
            if (rc != KEYVALUESTORE_OK) { allCountOk = 0; break; }
            keyvaluestore_iterator_first(iter);
            int count = 0;
            while (!keyvaluestore_iterator_eof(iter)) {
                count++;
        ...
        int isolOk = 1;
        void* got = NULL; int glen = 0;
        rc = keyvaluestore_cf_get(cfs[1], "cf0-key-0", 9, &got, &glen);
        if (rc != KEYVALUESTORE_NOTFOUND) isolOk = 0;
        if (got) sqliteFree(got);

        for (i = 0; i < nCF; i++) keyvaluestore_cf_close(cfs[i]);
...
static void test_growing_values(void) {
    ...
    if (rc == KEYVALUESTORE_OK) {
        ...
        for (i = 0; i < nSizes; i++) {
            ...
            rc = keyvaluestore_put(kv, key, (int)strlen(key), val, vlen);
            if (rc != KEYVALUESTORE_OK) { allOk = 0; free(val); break; }

            void* got = NULL; int glen = 0;
            rc = keyvaluestore_get(kv, key, (int)strlen(key), &got, &glen);
            if (rc != KEYVALUESTORE_OK || glen != vlen || memcmp(got, val, vlen) != 0) {
        ...
        char* errMsg = NULL;
        int ic = keyvaluestore_integrity_check(kv, &errMsg);
        if (ic != KEYVALUESTORE_OK) allOk = 0;
        if (errMsg) sqliteFree(errMsg);
...
static void test_mode_switch_persistence(void) {
    ...
    rc = keyvaluestore_open(DB_FILE, &kv, KEYVALUESTORE_JOURNAL_DELETE);
    if (rc != KEYVALUESTORE_OK) { print_result("Mode switch persistence (DELETE<->WAL)", 0); return; }
    keyvaluestore_put(kv, "del_key", 7, "del_value", 9);
    keyvaluestore_close(kv);

    rc = keyvaluestore_open(DB_FILE, &kv, KEYVALUESTORE_JOURNAL_WAL);
    if (rc != KEYVALUESTORE_OK) { print_result("Mode switch persistence (DELETE<->WAL)", 0); return; }

    void* got = NULL; int glen = 0;
    rc = keyvaluestore_get(kv, "del_key", 7, &got, &glen);
    int delOk = (rc == KEYVALUESTORE_OK && glen == 9 && memcmp(got, "del_value", 9) == 0);
    if (got) { sqliteFree(got); } got = NULL;

    keyvaluestore_put(kv, "wal_key", 7, "wal_value", 9);
    keyvaluestore_close(kv);
    ...
    rc = keyvaluestore_get(kv, "del_key", 7, &got, &glen);
    int dk = (rc == KEYVALUESTORE_OK && glen == 9);
    if (got) { sqliteFree(got); } got = NULL;

    rc = keyvaluestore_get(kv, "wal_key", 7, &got, &glen);
    int wk = (rc == KEYVALUESTORE_OK && glen == 9 && memcmp(got, "wal_value", 9) == 0);
    if (got) { sqliteFree(got); } got = NULL;
...
int main(void) {
    ...
    test_write_storm(KEYVALUESTORE_JOURNAL_DELETE, "Write storm DELETE mode (100K writes + integrity)");
    test_write_storm(KEYVALUESTORE_JOURNAL_WAL, "Write storm WAL mode (100K writes + integrity)");
