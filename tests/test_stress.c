...
**
** Modeled after SQLite's own storage-engine validation methodology:
**
**   1. Edge cases & boundary values
**   2. High-volume write storms with periodic integrity checks
**   3. Large-dataset fill, verify, and bulk-delete
**   4. Crash-recovery simulation (close mid-transaction, reopen)
**   5. Rapid open/close cycle resilience
**   6. Cross-configuration (DELETE + WAL journal modes)
**   7. Concurrent reader/writer stress
**   8. Iterator full-scan correctness
**   9. Transaction cycling stress
**  10. Mixed-workload (random put/get/delete interleaved)
*/
...
static void test_edge_empty_value(void){
  /* SQLite validates that zero-length payloads round-trip correctly */
  cleanup(DB_FILE);
  KVStore *kv = NULL;
...
static void test_edge_binary_key_with_nulls(void){
  /* Keys containing embedded NUL bytes must be handled as blobs */
  cleanup(DB_FILE);
  KVStore *kv = NULL;
  int rc = kvstore_open(DB_FILE, &kv, 0, KVSTORE_JOURNAL_DELETE);
  int ok = 0;
...
static void test_edge_large_key_value(void){
  /* Boundary: large key (1 KB) and large value (1 MB) */
  cleanup(DB_FILE);
  KVStore *kv = NULL;
  int rc = kvstore_open(DB_FILE, &kv, 0, KVSTORE_JOURNAL_DELETE);
  int ok = 0;
...
static void test_edge_overwrite_same_key(void){
  /* Repeated overwrites of the same key must always yield last value */
  cleanup(DB_FILE);
  KVStore *kv = NULL;
  int rc = kvstore_open(DB_FILE, &kv, 0, KVSTORE_JOURNAL_DELETE);
  int ok = 0;
...
static void test_write_storm(int journalMode, const char *label){
  ...
  if( rc == KVSTORE_OK ){
    ...
    for(i = 0; i < WRITE_STORM_N; i++){
      ...
      /* Periodic integrity check every 25k rows (SQLite-style) */
      if( (i % 25000) == 0 && i > 0 ){
        kvstore_commit(kv);
        char *errMsg = NULL;
    ...
    /* Spot-check: verify a handful of keys */
    int spotOk = 1;
...
/* ================================================================
** 3. LARGE DATASET: FILL, FULL-SCAN VERIFY, BULK DELETE
** ================================================================ */
...
static void test_large_dataset(void){
  ...
  if( rc == KVSTORE_OK ){
    ...
    /* Phase 2: Full-scan iterator verify */
    t0 = now_sec();
    ...
    /* Phase 3: Spot-verify random keys */
    int verifyOk = 1;
    srand(42);
    for(i = 0; i < 100; i++){
    ...
    /* Phase 4: Bulk delete first half */
    t0 = now_sec();
    kvstore_begin(kv, 1);
    ...
    /* Verify second half still intact */
    int halfOk = 1;
    for(i = 0; i < 50; i++){
...
static void test_crash_recovery_uncommitted(void){
  /*
  ** SQLite guarantees: if a transaction is not committed,
  ** the database reverts to its prior consistent state on reopen.
  **
  ** Simulate by closing the handle mid-transaction (without commit).
  */
  cleanup(DB_FILE);
  KVStore *kv = NULL;
  int rc;

  /* Step 1: Write some committed data */
  ...
  /* Step 2: Reopen, begin write txn, write uncommitted data, then close WITHOUT commit */
  rc = kvstore_open(DB_FILE, &kv, 0, KVSTORE_JOURNAL_DELETE);
  if( rc != KVSTORE_OK ){ print_result("Crash recovery: uncommitted txn", 0); return; }
  kvstore_begin(kv, 1);
  ...
  /* Step 3: Reopen and verify: "safe" exists, "unsafe" does not */
  rc = kvstore_open(DB_FILE, &kv, 0, KVSTORE_JOURNAL_DELETE);
  if( rc != KVSTORE_OK ){ print_result("Crash recovery: uncommitted txn", 0); return; }
...
static void test_rapid_open_close(void){
  /*
  ** SQLite's testing validates that rapidly opening and closing
  ** a database doesn't corrupt state or leak resources.
  */
  cleanup(DB_FILE);
  KVStore *kv = NULL;
  int rc;
  ...
  /* Rapid open/close with read verification */
  int i;
  for(i = 0; i < cycles; i++){
    rc = kvstore_open(DB_FILE, &kv, 0, KVSTORE_JOURNAL_DELETE);
    ...
    /* Every 50th cycle, write + commit to stress journal */
    if( (i % 50) == 0 ){
      char val[32];
...
/* ================================================================
** 6. CROSS-CONFIGURATION: BOTH JOURNAL MODES
** ================================================================ */

static void test_cross_config(void){
  /*
  ** SQLite's TH3 tests key operations across all journal modes.
  ** We test the same sequence in DELETE and WAL.
  */
  ...
  for(m = 0; m < 2; m++){
    ...
    /* Insert, read, update, delete, re-insert */
...
typedef struct {
  ...
  int id;
  int ops;
  int errors;       /* real (non-BUSY) errors */
...
static void *writer_thread(void *arg){
  ...
  for(i = 0; i < ta->ops; i++){
    ...
    /*
    ** SQLite WAL allows only one writer at a time.  SQLITE_BUSY is the
    ** expected response when another connection holds the write lock.
    ** We retry with exponential back-off; if still busy after retries
    ** we skip the op (this is normal contention, not an error).
    */
    int retries = 0;
    int ok = 0;
    do {
    ...
    if( ok ){
      ...
      ta->busySkips++;  /* BUSY contention, not an error */
    }
  }
  kvstore_close(kv);
  return NULL;
...
static void test_concurrent_stress(void){
  ...
  if( rc == KVSTORE_OK ){
    ...
    /* Verify that successfully-written keys are actually readable */
    int verifyOk = 1;
    for(i = 0; i < 2 && verifyOk; i++){
      /* Check the first key each writer would have written */
      char key[32];
      snprintf(key, sizeof(key), "t%d-key-0", i);
      int exists = 0;
      kvstore_exists(kv, key, (int)strlen(key), &exists);
      /* It should exist if that writer succeeded at least once */
...
static void test_iterator_correctness(void){
  /*
  ** Verify iterator visits every key exactly once and in order
  ** (btree keys are sorted by FNV-1a hash, so we check count).
  */
  cleanup(DB_FILE);
  KVStore *kv = NULL;
  int rc = kvstore_open(DB_FILE, &kv, 0, KVSTORE_JOURNAL_DELETE);
...
static void test_transaction_cycling(void){
  /*
  ** SQLite tests rapid begin/commit cycling to stress journal management.
  ** Each cycle writes one key and commits.
  */
  cleanup(DB_FILE);
  KVStore *kv = NULL;
  int rc = kvstore_open(DB_FILE, &kv, 0, KVSTORE_JOURNAL_DELETE);
...
static void test_rollback_cycling(void){
  /*
  ** Rapid begin/rollback cycling — must not corrupt or leak.
  */
  cleanup(DB_FILE);
  KVStore *kv = NULL;
  int rc = kvstore_open(DB_FILE, &kv, 0, KVSTORE_JOURNAL_DELETE);
  ...
  if( rc == KVSTORE_OK ){
    ...
    /* Anchor must survive, ephemeral must not exist */
    void *got = NULL; int glen = 0;
...
/* ================================================================
** 10. MIXED WORKLOAD (RANDOM PUT/GET/DELETE)
** ================================================================ */
...
static void test_mixed_workload(void){
  ...
  if( rc == KVSTORE_OK ){
    ...
    int errors = 0;
    int keySpace = 10000; /* reuse keys within this range */
    char key[32], val[128];
...
static void test_cf_stress(void){
  ...
  if( rc == KVSTORE_OK ){
    /* Create multiple CFs, write to each, verify isolation */
    int nCF = 10;
    ...
    /* Verify each CF has exactly 500 keys via iterator */
    int allCountOk = 1;
    ...
    /* Spot-check cross-CF isolation: key from CF 0 shouldn't appear in CF 1 */
    int isolOk = 1;
    void *got = NULL; int glen = 0;
...
static void test_growing_values(void){
  /*
  ** Test with values that grow from tiny (1 byte) to large (512 KB).
  ** This stresses btree page overflow handling.
  */
  cleanup(DB_FILE);
  KVStore *kv = NULL;
  int rc = kvstore_open(DB_FILE, &kv, 0, KVSTORE_JOURNAL_WAL);
...
/* ================================================================
** 13. PERSISTENCE ACROSS MODE SWITCH
** ================================================================ */

static void test_mode_switch_persistence(void){
  /*
  ** Write in DELETE mode, close, reopen in WAL mode, verify data,
  ** write more, close, reopen in DELETE mode, verify all data.
  */
  cleanup(DB_FILE);
  KVStore *kv = NULL;
  int rc;
  ...
  /* Phase 3: Reopen in DELETE, verify both keys */
  rc = kvstore_open(DB_FILE, &kv, 0, KVSTORE_JOURNAL_DELETE);
  if( rc != KVSTORE_OK ){ print_result("Mode switch persistence (DELETE<->WAL)", 0); return; }
