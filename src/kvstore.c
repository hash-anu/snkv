...
/*
** Opt-in mutex guards for pKV->pMutex.
** When fullMutex=0 (default), pMutex is NULL and these expand to nothing.
** When fullMutex=1, they acquire/release the recursive mutex, serialising
** all API calls on a shared KVStore* handle.
*/
#define KV_ENTER(p) do { if((p)->pMutex) sqlite3_mutex_enter((p)->pMutex); } while(0)
#define KV_LEAVE(p) do { if((p)->pMutex) sqlite3_mutex_leave((p)->pMutex); } while(0)
...
int kvstore_open_v2(
  ...
  *ppKV = NULL;
  ...
  /* Create main mutex only when the caller opts in to shared-handle safety.
  ** pMutex stays NULL for the default single-handle-per-thread pattern;
  ** KV_ENTER/KV_LEAVE check for NULL so they become no-ops in that case. */
  if( pConfig && pConfig->fullMutex ){
  ...
  if( pCF->hasTtl && pCF->nTtlActive > 0 && pCF->pTtlKeyCF ){
    ...
    if( rck == SQLITE_OK && nTtlVal == 8 ){
      ...
      if( kvstore_now_ms() >= expireMs ){
        ...
        pCF->pReadCur = NULL;
        if( pKV->inTrans == 2 ){
          /* Caller owns this write transaction (via kvstore_begin) — do not
          ** commit or mutate it here. Defer reclamation to a later get or
          ** an explicit purge once the caller's transaction has ended. */
        }else{
          if( pKV->inTrans == 1 ){
            kvstoreInvalidateReadCursors(pKV);
            sqlite3BtreeCommit(pKV->pBt); pKV->inTrans = 0;
  ...
  if( rc == SQLITE_OK && found &&
    ...
    if( rck == SQLITE_OK && nTtlVal == 8 ){
      ...
      if( kvstore_now_ms() >= expireMs ){
        ...
        pCF->pReadCur = NULL;
        if( pKV->inTrans == 2 ){
          /* Caller owns this write transaction — defer reclamation, see
          ** kvstore_cf_get_internal for the same guard and rationale. */
        }else{
          if( pKV->inTrans == 1 ){
            kvstoreInvalidateReadCursors(pKV);
            sqlite3BtreeCommit(pKV->pBt); pKV->inTrans = 0;
...
int kvstore_sync(KVStore *pKV){
  ...
  if( pKV->inTrans == 2 ){
    /* An explicit write transaction (opened via kvstore_begin) is active.
    ** It belongs to the caller — committing it here to "sync" would make
    ** their in-progress writes durable and unrollbackable behind their
    ** back. Require them to commit or rollback first. */
    kvstoreSetError(pKV, "cannot sync: an explicit transaction is active; "
