...
int main() {
    ...
    rc = kvstore_open("hello.db", &pKV, 0);
    if (rc != KVSTORE_OK) {
...
int main() {
    KVStore *pKV;
    kvstore_open("inventory.db", &pKV, 0);
...
int main() {
    ...
    // Open database
    kvstore_open("ecommerce.db", &pKV, 0);
...
int main() {
    KVStore *pKV;
    kvstore_open("multi_cf.db", &pKV, 0);
...
int main() {
    KVStore *pKV;
    kvstore_open("sessions.db", &pKV, 0);
...
int config_init(ConfigManager *mgr, const char *env) {
    int rc = kvstore_open("config.db", &mgr->pKV, 0);
    if (rc != KVSTORE_OK) {
        return rc;
    }
...
int main() {
    ...
    kvstore_open("multithreaded.db", &pKV, 0);
...
int main() {
    ...
    kvstore_open("versioned.db", &pKV, 0);
...
int main() {
    ...
    kvstore_open("users_indexed.db", &pKV, 0);
